...
def get_ecs_service_ips(cluster_name, service, region, profile, private):
    ...
    try:
        ...
        if not services:
            click.echo(
                f"No services found in cluster {cluster_name}", err=True
            )
            return
